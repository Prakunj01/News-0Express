...
const Dashboard = () => {
    ...
    const deleteNews = async () => {
        try {
            await axios.delete(`http://localhost:5000/api/v1/news/delete/${deleteConfirmation}`, { withCredentials: true });
            setNews(news.filter(n => n._id !== deleteConfirmation));
            setDeleteConfirmation(null); // Clear delete confirmation
            toast.success('News item deleted successfully'); // Show success toast
        } catch (error) {
            console.log(error);
            // Handle error
            toast.error('Failed to delete news item'); // Show error toast
        }
    };
                            ...
                            news.length > 0 && news.slice((page - 1) * parPage, page * parPage).map((n, i) => <tr key={i} className='bg-white border-b' >
                                ...
                                <td className='px-6 py-4'>
                                    <div className='flex justify-start items-center gap-x-4 text-white'>
                                        <Link to={`/api/v1/news/${n._id}`} className='p-[6px] bg-green-500 rounded hover:shadow-lg hover:shadow-green-500/50'><FaEye /></Link>

                                        <Link to={`editNews/${n._id}`} className='p-[6px] bg-yellow-500 rounded hover:shadow-lg hover:shadow-yellow-500/50'><FaEdit />
                                        </Link>
